var
    ...
    // TMP Folder
    tmpFolder = path.join(os.tmpDir(), 'FreedomFlix'),
...
if( ! Settings.get('disclaimerAccepted') ) {
    $('.freedom-disclaimer').removeClass('hidden');

    $('.freedom-disclaimer .btn.confirmation.continue').click(function(event){
        event.preventDefault();
        Settings.set('disclaimerAccepted', 1);
        $('.freedom-disclaimer').addClass('hidden');
    });
    $('.freedom-disclaimer .btn.confirmation.quit').click(function(event){
        event.preventDefault();

        // We need to give the tracker some time to send the event
        // Also, prevent multiple clicks
        if( $('.freedom-disclaimer').hasClass('quitting') ){ return; }
        $('.freedom-disclaimer').addClass('quitting');
